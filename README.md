...
ログインしてオンラインのユーザー一覧を確認し、相手を選んで通話できる。グループ通話（Zoomライクな画面共有）や自己映像PiP表示にも対応。
...
- True Color (24-bit) 対応ターミナル (Alacritty, WezTerm, iTerm2 など)
...
[Client A (TUI)] ───────── Direct LiveKit Cloud ─────────> [Client B (TUI)]
       ...
       │  Room "call_*" or user room: 音声/映像 (WebRTC)         │
       │                                                       │
...
- 1対1通話: **call\_{caller}\_{callee}** ルームが動的生成され、音声・映像が流れる
- グループ通話: `[j]` キーで任意のルーム名を指定し、同じルームに入ったメンバー全員と接続
...
- **音声が届かない場合**: マイクが正しく認識されているか確認してください。`cargo run` のログに `[audio]` の診断情報が出力されます。
- **ALSAエラーが大量に出る場合**: PulseAudio/PipeWire環境では問題ありません。`find_input_device` が自動的に適切なデバイスを選択します。
