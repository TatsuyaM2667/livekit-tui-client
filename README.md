...
ログインしてオンラインのユーザー一覧を確認し、相手を選んで通話できます。
...
- シグナリングサーバーは不要。**LiveKit Cloud** に直接接続します
- 全クライアントが永続的に **lobby** ルームに参加し、在籍管理とシグナリング（発信・着信通知）を **LiveKit Data Channel** 上で行います
- 通話開始時に **call_{caller}_{callee}** ルームが動的生成され、音声・映像が流れます
...
シグナリングサーバーは不要です。各クライアントが LiveKit Cloud に直接接続します。
...
1. 各デバイスの `.env` に同じ LiveKit Cloud の認証情報を設定します。
2. 各デバイスで `cargo run --bin client` を実行し、別々のユーザー名でログインします。
3. ユーザー一覧から相手を選択して `[Enter]` を押すと着信通知が飛び、相手が `[y]` を押すと通話が始まります。
